...
router.get('/', async (req, res) => {
  try {
    ...
    const { project_id, calculation_type } = req.query;
    let queryText = `
...
router.post('/', async (req, res) => {
  try {
    ...
    const userId = await getUserId(req.auth.userId);
    const { project_id, initial_investment, current_value, time_period_months, calculation_type, notes } = value;
    ...
    const roi_percentage = calculateROI(initial_investment, current_value, time_period_months, calculation_type);
    ...
    logger.info('New ROI calculation created for user ${userId}`);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const newInitialInvestment = initial_investment !== undefined ? initial_investment : current.initial_investment;
    const newCurrentValue = current_value !== undefined ? current_value : current.current_value;
    const newTimePeriod = time_period_months !== undefined ? time_period_months : current.time_period_months;
    const newCalculationType = calculation_type || current.calculation_type;
    const newNotes = notes !== undefined ? notes : current.notes;

    // Recalculate ROI
    const roi_percentage = calculateROI(newInitialInvestment, newCurrentValue, newTimePeriod, newCalculationType);
...
router.post('/calculate', async (req, res) => {
  try {
    ...
    const roi_percentage = calculateROI(initial_investment, current_value, time_period_months, calculation_type);
    const gain = current_value - initial_investment;
    const gain_percentage = (gain / initial_investment) * 100;

    let annualized_return = null;
    if (time_period_months && time_period_months > 0) {
...
router.get('/stats/overview', async (req, res) => {
  try {
    ...
    const totalInvested = parseFloat(stats[4].rows[0].total_invested) || 0;
    const totalValue = parseFloat(stats[5].rows[0].total_value) || 0;
