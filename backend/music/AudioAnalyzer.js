...
export class AudioAnalyzer {
    ...
    inferMood(pitchAnalysis, tempoAnalysis, harmonicAnalysis) {
        const isMinor = pitchAnalysis.scale?
      .includes('minor');
        const isSlow = tempoAnalysis.bpm < 100;
        const isDissonant = harmonicAnalysis.consonance < 0.5;

        if (isMinor && isSlow) return 'melancholy';
        if (isMinor && !isSlow) return 'dramatic';
        if (!isMinor && tempoAnalysis.bpm > 120) return 'upbeat';
        if (!isMinor && isSlow) return 'peaceful';
        if (isDissonant) return 'tense';

        return 'neutral';
    }

    /**
     * Calcule la dansabilité
     */
    calculateDanceability(tempoAnalysis, rhythmAnalysis) {
        let danceability = 0;

        // BPM optimal pour danser (100-140)
        if (tempoAnalysis.bpm >= 100 && tempoAnalysis.bpm <= 140) {
            danceability += 0.4;
        }

        // Stabilité du tempo
        if (tempoAnalysis.stability > 0.8) {
            danceability += 0.3;
        }

        // Complexité rythmique modérée
        if (rhythmAnalysis.complexity > 0.3 && rhythmAnalysis.complexity < 0.8) {
            danceability += 0.3;
        }

        return Math.min(danceability, 1.0);
    }

    /**
     * Calcule la valence (positivité)
     */
    calculateValence(pitchAnalysis, harmonicAnalysis) {
        let valence = 0.5; // Neutre par défaut

        // Tonalité majeure = plus positif
        if (pitchAnalysis.scale?.includes('major')) {
            valence += 0.3;
        } else if (pitchAnalysis.scale?.includes('minor')) {
            valence -= 0.3;
        }

        // Consonance = plus positif
        valence += (harmonicAnalysis.consonance - 0.5) * 0.4;

        return Math.max(0, Math.min(1, valence));
    }

    /**
     * Calcule l'excitation/arousal
     */
    calculateArousal(tempoAnalysis, dynamicAnalysis) {
        let arousal = 0;

        // Tempo rapide = plus excitant
        arousal += Math.min(tempoAnalysis.bpm / 180, 1) * 0.6;

        // Volume élevé = plus excitant
        arousal += dynamicAnalysis.loudness * 0.4;
     ...
     */
    assessAnalysisQuality(analyses) {
        ...
        const averageConfidence = validAnalyses > 0 ? totalConfidence / validAnalyses  :
       0;
